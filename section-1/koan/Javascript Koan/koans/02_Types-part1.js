describe('type에 대해서 학습합니다.', function () {
  ...
  */
  ...
  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(1 + '1').to.equal('11');   // number 타입과 string 타입의 숫자를 '+' 연산자로 연산을 하면 string 타입으로 변환 되고 나란히 붙어서 출력 된다.
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(123 - '1').to.equal(122);  // + 연산자와 다르게
                                      // - (마이너스), * (곱하기), / (나누기) 연산자를 사용하면 일반적인 연산 방식으로 적용된다.
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(1 + true).to.equal(2);  // boolean 타입은 number 타입과 연산될 때 
                                   // true = 1 / false = 0 으로 적용된다.
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect('1' + true).to.equal('1true'); // 위와 반대로 string 타입 + boolean 타입 연산의 결과는
                                          // string으로 결과가 출력된다.
  });
