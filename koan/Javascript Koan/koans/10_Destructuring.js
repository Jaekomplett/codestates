describe('구조 분해 할당(Destructing Assignment)에 관해서', () => {
  ...
  it('rest/spread 문법을 배열 분해에 적용할 수 있습니다', () => {
    ...
    expect(start).to.eql('code')                       //TODO 왜 start === string ? rest === array? 
  ...
  it('rest/spread 문법을 객체 분해에 적용할 수 있습니다 #3', () => {
    ...
    expect(changedUser).to.eql({...user, name: '박해커', age: 20})

    expect(overwriteChanges).to.eql({name: '박해커', age: 20, ...user})

    expect(changedDepartment).to.eql({...user, company: {...user.company, department: 'Marketing'}})
  })
})