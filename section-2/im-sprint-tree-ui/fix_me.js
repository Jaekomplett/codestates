...
function createTreeView(menu, currentNode) {
  for (let i = 0; i < menu.length; i++) { // 전체 메뉴의 수를 반복 조회
    const li = document.createElement('li'); // 메뉴 li 생성
    if (menu[i].children) { // 메뉴 li 중에 자식 노드가 있다면
      const span = document.createElement('span'); // span 태그 생성
      span.textContent = menu[i].name;
      const input = document.createElement('input'); // checkbox input 태그 생성
      input.type = 'checkbox';

      const ul = document.createElement('ul'); // 4) 자식 노드 생성 
